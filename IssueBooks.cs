                ...
                if(con.State != ConnectionState.Open)
                {
                    try
                    {
                        ...
                        string insertData = "INSERT INTO issues " +
                            "(issue_id, full_name, contact, email, book_id, book_title, author, status, issue_date, return_date, date_insert) " +
                            "VALUES(@issueID, @fullname, @contact, @email, @bookID, @bookTitle, @author, @status, @issueDate, @returnDate, @dateInsert)";
                        ...
                        using(MySqlCommand cmd = new MySqlCommand(insertData, con))
                        {
                            ...
                            int bookId = Convert.ToInt32(bookIssue_bookTitle.SelectedValue);
